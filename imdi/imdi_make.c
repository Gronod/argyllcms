...
main(int argc, char *argv[]) {
	...
	int use64 = 0;
#endif
	static char dirname[MAXNAMEL+1+1] = "";   /* Output directory name */
	static char temp[MAXNAMEL+100+1];		/* Buffer to compose filenames in */
	...
	for (dn = 0; dn < tnd; dn++) {
		...
		for (cb = 0, ncb = 1; cb < ncb; cb++) {
			int nalg, alg;
			char ofname[100] = "indiv";		// ~~~999 this isn't right...
