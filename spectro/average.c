 ...
 * Date:   18/1/2011
 *
 * Copyright 2005, 2010, 2011, 2025 Graeme W. Gill
 * All rights reserved.
 *
 ...
 * TTBD:
	...
	Geometric mean is limited to just 3 channels.
 */
...
int main(int argc, char *argv[]) {
	...
	int dogeom = 0;				/* Do geometric median of PCS or Dev, 2 = Lab, 3 = PCS */
	...
	for (fa = 1;fa < argc;fa++) {
		...
		if (argv[fa][0] == '-')	{	/* Look for any flags */
			...
			/* Geometric Median of PCS or Dev */
			else if (argv[fa][1] == 'g') {
				dogeom = 1;
			}
	...
	/* Check the device/profile type tag to see if input type */
	for (n = 0; n < inps[0].c->ntables; n++) {
		if ((dti = inps[n].c->find_kword(inps[n].c, 0, "DEVICE_CLASS")) < 0) {
			if (n == 0)
				inp = 0;		/* Default to output */
			continue;
		}
	
		if (strcmp(inps[n].c->t[0].kdata[dti],"INPUT") == 0
		 || strcmp(inps[n].c->t[0].kdata[dti],"EMISINPUT") == 0) {
			if (n == 0)
				inp = 1;
			else if (inp != 1)
				error ("Input file '%s' has conflicting DEVICE_CLASS",inps[n].name);
		} else {
			if (n == 0)
				inp = 0;
			else if (inp != 0)
...
//printf("~1 table %d, adding keyword '%s'\n",n,inps[0].c->t[n].ksym[i]);
	...
	if (!haspcs[0] && !haspcs[1]) {
		if (inp)
			error("Need a PCS to average for input device chart");
		else
	...
	if (inp && dogeom != 0) {
		...
		if (nchan != 3)
			error ("Can't do geometric mean for %d device channels",nchan);
		
		if (dogeom > 1) {
			warning("Geometric mean of XYZ or Lab ignored for input type since device values are being averaged");
			dogeom = 1;
		}
	}
		...
		for (i = 0; i < inps[0].c->t[0].nsets; i++) {
			...
			for (j = 0; j < inps[0].c->t[0].nfields; j++) {
				...
				if (inp) {
					...
					/* Locate any patches (including starting patch) with matching PCS values */
					npat = 0;
					for (k = i; k < inps[0].c->t[0].nsets; k++) {
						int mm;
	
						/* Check if the PCS values match */
						for (mm = 0; mm < 2; mm++) {
							if (haspcs[mm] == 0)
								continue;
							for (e = 0; e < nchan; e++) {
								...
								diff = *((double *)inps[0].c->t[0].fdata[i][pcsix[mm][e]])
								     - *((double *)inps[0].c->t[0].fdata[k][pcsix[mm][e]]);
			...
			/* Override per-component average/median if Device Geometric Median */
			if (inp && dogeom && (haspcs[0] || haspcs[1])) {
				double res[3];
				...
				for (k = i; k < inps[0].c->t[0].nsets; k++) {
					...
					/* (nchan == 3 when inp and dogeom) */
					v3list[npat][0] = *((double *)inps[0].c->t[0].fdata[k][chix[0]]);
					v3list[npat][1] = *((double *)inps[0].c->t[0].fdata[k][chix[1]]);
					v3list[npat][2] = *((double *)inps[0].c->t[0].fdata[k][chix[2]]);

					npat++;
				}
				geommed(res, v3list, npat);

				*((double *)ocg->t[0].fdata[n][chix[0]]) = res[0];
				*((double *)ocg->t[0].fdata[n][chix[1]]) = res[1];
				*((double *)ocg->t[0].fdata[n][chix[2]]) = res[2];

			/* Override per-component average/median if PCS Geometric Median */
			} else if (!inp && dogeom && (haspcs[0] || haspcs[1])) {
				double res[3];
			...
			if (domerge) {
				...
				for (i = 0; i < inps[n].c->t[0].nsets; i++) {
					if (inp) {
						...
						/* Check that the PCS values match the first file */
						for (k = 0; k < 2; k++) {
							if (haspcs[k] == 0)
								continue;
							for (j = 0; j < 3; j++) {
								double diff;
								diff = *((double *)inps[0].c->t[0].fdata[i][pcsix[k][j]])
								     - *((double *)inps[n].c->t[0].fdata[i][pcsix[k][j]]);
			
								if (fabs(diff) > 0.001)
									error ("File '%s' set %d has field '%s' value that differs from '%s'",
							       inps[n].name, i+1, inps[n].c->t[0].fsym[pcsix[k][j]], inps[0].name);
							}
						}
					} else {
		...
		if (!domerge) {
			...
			if (inp && dogeom && (haspcs[0] || haspcs[1])) {
				...
				for (i = 0; i < inps[0].c->t[0].nsets; i++) {
					...
					npat = 0;
					for (n = 0; n < nchan; n++) {
						v3list[npat][0] = *((double *)inps[n].c->t[0].fdata[i][chix[0]]);
						v3list[npat][1] = *((double *)inps[n].c->t[0].fdata[i][chix[1]]);
						v3list[npat][2] = *((double *)inps[n].c->t[0].fdata[i][chix[2]]);
						npat++;
					}
					geommed(res, v3list, npat);

					*((double *)ocg->t[0].fdata[i][chix[0]]) = res[0];
					*((double *)ocg->t[0].fdata[i][chix[1]]) = res[1];
					*((double *)ocg->t[0].fdata[i][chix[2]]) = res[2];
				}
...
/* Compute Geometric Median of PCS or Dev3 values */
