...
//printf("~1  icc set ilimit %f uilimit %f\n",*ilimit,*uilimit);
	...
	/* set pcpt ilimit values from user or ICC/MPP default */
	s->ilimit  = *ilimit;
	s->uilimit = *uilimit;

	/* Set a default pcpt ink limit if not set by user, ICC or MPP */
	if (s->ilimit < 0.0) {
		s->uilimit = (double)s->di + 0.1;	/* Default to no limit */
		s->ilimit = s->uilimit;
//printf("~1  set default pcpt ilimit %f uilimit %f\n",s->ilimit,s->uilimit);

		/* Convert ilimit to uilimit using cal */
		if ((s->cal != NULL) && s->ilimit >= 0.0 && s->ilimit < (double)s->di) {
			s->uilimit = icxMaxUnderlyingLimit(s->cal, s->ilimit);
//printf("~1  recompute cal pcpt uilimit %f\n",s->uilimit);
		}
	}
...
int main(int argc, char *argv[]) {
	...
	static char fname[MAXNAMEL+10+1] = { 0 };	/* Output file base name */
	static char pname[MAXNAMEL+1] = { 0 };		/* Device profile name */
	static char cname[MAXNAMEL+1] = { 0 };		/* Device calibration file name */
	static char wdname[MAXNAMEL+10+1] = { 0 };		/* Device diagnostic .wrl/.x3d name */
	static char wlname[MAXNAMEL+10+1] = { 0 };		/* Lab diagnostic .wrl/.x3d name */
