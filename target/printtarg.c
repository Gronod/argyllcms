...
#define T_PAD    0x8000			/* A padding color patch */
	...
	int media;					/* flag, nz if this is media */
...
/* If cal != NULL, apply calibration curves to value */
...
/* For no spacer, we return the density difference to the adjacent patches */
double setup_spacer(
...
int sptype,		/* Spacer type code, -1 = none, */
				...
				/* 2 = colored */
int usede,		/* Aim for maximum delta E rather than density (ie for i1Pro etc.) */
int cretm		/* Maximize contrast of row ends to media */
) {
	col *sc = NULL;	/* Spacer chosen */
	double dd, pdd;		/* Spacer contrast, patch contrast */
...
//printf("~1 setting spacer between %s (%s) and %s (%s)\n",pp->loc, pp->id, cp->loc, cp->id);
	...
	if (usede) {
		...
		/* Compute the density rather than DE */
		if (pp->nmask == ICX_W
...
//printf("~1 returning spacer contrast %f + patch contrast %f\n",dd,pdd);
	...
	if (cretm && (pp->media || cp->media))
		return pdd;					/* Return just patch contrast */
...
int cretm			/* Increase contrast of row ends to media */
) {
	int i;
	...
	/* Setup initial contrast check */
	{
		...
		/* (wnd = Worst neigborhood contrast density) */
		...
		for (i = 0; i < npat; i++) {
			...
			if (j == (tpprow-1) || i == (npat-1)) { /* Last in row or last patch */
				if ((j == (tpprow-1) || i == (npat-1)) && domaxmin == 1)
					...
					np = media;		/* Media will be after last patch */
			} else {
				np = &cols[rix[i+1]];
			}
			...
			/* Setup pointers on patch */
			cp->nc[0] = pp;
			cp->nc[1] = np;
			cp->oc = op;

			/* Compute initial worst case contrast */
			cp->wnd = setup_spacer(NULL, pp, cp, pcol, spacer, usede, cretm);
			tt      = setup_spacer(NULL, cp, np, pcol, spacer, usede, cretm);
			if (tt < cp->wnd)
				cp->wnd = tt;
			if (cp != op) {
		...
		for (temp = tstart; temp > tend; temp *= trate) {
			...
			for (ii = 0; ii < itlim ; ii++) { 
				...
				de = setup_spacer(NULL, p2->nc[0], p1, pcol, spacer, usede, cretm);
				tt = setup_spacer(NULL, p1, p2->nc[1], pcol, spacer, usede, cretm);
				if (tt < de)
					de = tt;
				...
				tt = setup_spacer(NULL, p1->nc[0], p2, pcol, spacer, usede, cretm);
				if (tt < de)
					de = tt;
				tt = setup_spacer(NULL, p2, p1->nc[1], pcol, spacer, usede, cretm);
				if (tt < de)
					de = tt;
...
//printf("~1 temp = %f, ii = %d, swapping %d and %d\n",temp,ii,p1->i, p2->i);
					...
					/* of the situation if they are neigbors themselves */
					tp0 = p1->nc[0];
					tp1 = p2->nc[0];
					if (tp0 == p1)
						tp0 = p2;
					...
					p1->wnd = setup_spacer(NULL, p1->nc[0], p1, pcol, spacer, usede, cretm);
					tt      = setup_spacer(NULL, p1, p1->nc[1], pcol, spacer, usede, cretm);
					if (tt < p1->wnd)
						p1->wnd = tt;
					if (p1 != p1->oc) {
					...
					p2->wnd = setup_spacer(NULL, p2->nc[0], p2, pcol, spacer, usede, cretm);
					tt      = setup_spacer(NULL, p2, p2->nc[1], pcol, spacer, usede, cretm);
					if (tt < p2->wnd)
						p2->wnd = tt;
					if (p2 != p2->oc) {
					...
					/* ->wnd due to it's opposite having changed. !!!) */
...
int *p_npat			/* Return number of patches including padding */
	...
	int cretm = 0;		/* Increase contrast of row ends to media */
	...
	int *rix;	/* [npat+1] Random index lookup (Logical -> patch index) */
	...
	if (itype == instDTP20) { 	/* Xrite DTP20 */
		...
		cretm = 1;				/* Increase contrast of row ends to media */
...
//printf("~1 padded no of patches = %d\n", npat);
	...
	setup_randix(rix, npat, rand, rstart, verb, cols, pcol,
	             tpprow, spacer, needpc, domaxmin, media, usede, cretm);
...
//printf("~1 pir %d, ris %d, sip %d, pif %d\n", pir, ris, sip, pif);
		...
		if (spacer > 0) {
			setup_spacer(&sc, pp, cp, pcol, spacer, usede, 0);
...
int main(int argc, char *argv[]) {
	...
	static char inname[MAXNAMEL+10+1] = { 0 };	/* Input cgats file name */
	...
	static char outname[MAXNAMEL+10+1] = { 0 };	/* Output cgats file name */
	cgats *icg;				/* input cgats structure */
	cgats *ocg;				/* output cgats structure */
	xcal *cal = NULL;		/* printer calibration */ 
	instType itype = instI1Pro;		/* Default target instrument */
